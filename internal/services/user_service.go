...
func (u *UserService) GetProfile(id string) (*entities.User, error) {
	...
	for i := range user.Photos {
		if user.Photos[i].URL != "" && u.storageService != nil {
			user.Photos[i].URL = u.storageService.GetPublicURL(user.Photos[i].URL)
		}
	}

	return user, nil
}
func (u *UserService) UpdateProfile(userID uuid.UUID, data UpdateProfileRequest) error {
		...
		if err == nil && existingUser != nil {
			for _, existingPhoto := range existingUser.Photos {
				if !newPhotoIDs[existingPhoto.ID] {
					// Delete from S3
					if u.storageService != nil && existingPhoto.URL != "" {
						_ = u.storageService.DeleteFile(context.Background(), existingPhoto.URL)
					}
...
func (u *UserService) DeleteAccount(userID uuid.UUID) error {
