...
import (
	...
	_ "github.com/pipigendut/dating-backend/internal/entities"
...
func NewGroupHandler(groupSvc services.GroupService, storageService *services.StorageService) *GroupHandler {
	return &GroupHandler{
...
// @Produce      json
// @Security     BearerAuth
// @Param        request body dtov1.CreateGroupRequest true "Group details"
// @Success      200  {object}  base.BaseResponse{data=dtov1.GroupResponse} "Created group details"
// @Router       /groups [post]
func (h *GroupHandler) CreateGroup(c *gin.Context) {
	userID := c.MustGet("userID").(uuid.UUID)
	var req dtov1.CreateGroupRequest
	...
	if err != nil {
		base.Error(c, http.StatusInternalServerError, "Failed to create group", err.Error())
		return
	}
...
// @Produce      json
// @Security     BearerAuth
// @Success      200  {object}  base.BaseResponse{data=dtov1.GroupResponse} "Group details"
// @Failure      404  {object}  base.BaseResponse "Group not found"
...
func (h *GroupHandler) GetMyGroup(c *gin.Context) {
	...
	if err != nil {
		...
		base.Error(c, http.StatusInternalServerError, "Failed to fetch group", err.Error())
		return
	}

	// Map members
	members := make([]dtov1.UserResponse, len(group.Members))
	for i, m := range group.Members {
		if m.User != nil {
			members[i] = mapper.ToUserResponse(m.User, h.storageService)
		}
	}
...
// @Param        request body dtov1.InviteToGroupRequest true "Invite details"
// @Success      200  {object}  base.BaseResponse "Successfully invited"
// @Router       /groups/{id}/invite [post]
func (h *GroupHandler) InviteToGroup(c *gin.Context) {
	groupID, err := uuid.Parse(c.Param("id"))
	if err != nil {
		base.Error(c, http.StatusBadRequest, "Invalid group ID", err.Error())
		return
	}

	var req dtov1.InviteToGroupRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		base.Error(c, http.StatusBadRequest, "Invalid request", err.Error())
		return
	}
	...
	if err := h.groupService.InviteToGroup(c.Request.Context(), groupID, req.UserID); err != nil {
		base.Error(c, http.StatusInternalServerError, "Failed to invite to group", err.Error())
		return
	}
...
// @Security     BearerAuth
// @Param        id   path      string  true  "Group ID"
// @Success      200  {object}  base.BaseResponse{data=string} "Invite Link"
// @Router       /groups/{id}/invite-link [post]
func (h *GroupHandler) GenerateInviteLink(c *gin.Context) {
	groupIDStr := c.Param("id")
	groupID, err := uuid.Parse(groupIDStr)
	if err != nil {
		base.Error(c, http.StatusBadRequest, "Invalid Group ID", nil)
		return
	}
	...
	if err != nil || !isMember {
		base.Error(c, http.StatusForbidden, "Only group members can generate invite links", nil)
		return
	}

	invite, err := h.groupService.GenerateInviteLink(c.Request.Context(), groupID, userID)
	if err != nil {
		base.Error(c, http.StatusInternalServerError, "Failed to generate invite", err.Error())
		return
	}
...
// @Success      200  {object}  base.BaseResponse{data=map[string]interface{}}
// @Router       /group-invites/validate [get]
func (h *GroupHandler) ValidateInvite(c *gin.Context) {
	token := c.Query("token")
	if token == "" {
		base.Error(c, http.StatusBadRequest, "Token is required", nil)
		return
	}
...
func (h *GroupHandler) AcceptInvite(c *gin.Context) {
	...
	base.OK(c, "Successfully joined the group")
}
...
func (h *GroupHandler) KickMember(c *gin.Context) {
	...
	if err != nil {
		base.Error(c, http.StatusBadRequest, "Invalid user ID", err.Error())
		return
	}

	if err := h.groupService.KickMember(c.Request.Context(), groupID, ownerID, targetUserID); err != nil {
		base.Error(c, http.StatusInternalServerError, "Failed to kick member", err.Error())
		return
	}
...
func (h *GroupHandler) LeaveGroup(c *gin.Context) {
	...
	if err := h.groupService.LeaveGroup(c.Request.Context(), groupID, userID); err != nil {
		base.Error(c, http.StatusInternalServerError, "Failed to leave group", err.Error())
		return
	}
...
func (h *GroupHandler) DisbandGroup(c *gin.Context) {
	...
	if err := h.groupService.DisbandGroup(c.Request.Context(), groupID, ownerID); err != nil {
		base.Error(c, http.StatusInternalServerError, "Failed to disband group", err.Error())
		return
	}
